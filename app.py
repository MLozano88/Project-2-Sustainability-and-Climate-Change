...
def TopGDP():
    ...
    result = connection.execute(
        """SELECT country_code, country_name, color, "2015" FROM gdp_top20 ORDER BY "2015" DESC LIMIT 20;""")
    data = []
    for row in result:
        my_dict = {
            "country_code": row[0],
...
def TopRenewableEnergy():
    ...
    result = connection.execute(
        """SELECT country_code, country_name, color, "2015" FROM renewable_top20 ORDER BY "2015" DESC LIMIT 20;""")
    data = []
    for row in result:
        my_dict = {
            "country_code": row[0],
